...
def initialization_training_epoch(losses):
    ...
    loss = loss_function(latent_coeffs=latent_coeffs, cluster_attn=cluster_attn, firing_attn=firing_attn, mode='initialize_map')
    ...
    return losses, latent_coeffs, cluster_attn, firing_attn
...
def learn_initial_maps():
    ...
    for epoch in range(start_epoch, start_epoch + args.num_epochs):
        losses_train, latent_coeffs_train, cluster_attn_train, firing_attn_train = initialization_training_epoch(losses_train)
        losses_test, latent_coeffs_test, cluster_attn_test, firing_attn_test = initialization_evaluation(losses_test)
        if epoch % args.log_interval == 0 or epoch == start_epoch + args.num_epochs - 1:
            ...
            output_str = (
                ...
                f"Loss train: {cur_loss_train:.5f}, Loss test: {cur_loss_test:.5f},\n"
                f"lr: {lr:.5f}, smoothness_budget: {smoothness_budget_constrained.T}\n")
            write_log_and_model(output_str, output_dir, epoch, model=model)
            latent_factors = torch.matmul(latent_coeffs_train, Bspline_matrix).numpy()
            cluster_attn = cluster_attn_train.numpy()
            firing_attn = firing_attn_train.numpy()
            plot_outputs(latent_factors, cluster_attn, firing_attn, data_train.intensity, stim_time, output_dir, 'Train', epoch)
            latent_factors = torch.matmul(latent_coeffs_test, Bspline_matrix).numpy()
            cluster_attn = cluster_attn_test.numpy()
            firing_attn = firing_attn_test.numpy()
